...
public class HashMapMain {
	...
	public static void main(String[] args) {
		...
		for (int i=0;i<carObjectEntryArray.length;i++) { //carObjectEntryArray의 타입은 Object배열임
			...
			/*
			 * HashMap은 Entry 객체의 집합으로 이루어진 Set을 반환하는 entrySet() 메서드를 제공합니다. 
			 * 이 Entry 객체는 HashMap에 저장된 키-값 쌍을 나타냅니다.
			 *  따라서 entrySet()을 호출하면 Set<Map.Entry<K, V>> 형태의 결과를 얻게 됩니다.
			 *  
			 *  그러나 Set이 Entry 객체의 집합인 경우, toArray()를 호출하면 Object[] 형태의 배열이 반환됩니다
			 *  . 이 배열의 각 요소는 Map.Entry 타입의 객체입니다.			
			 *  어쨌든 EntrySet 객체의 집합인 set으로 변경된다는거지
			 */
			/**
			...
			Car car=(Car)carEntry.getValue();     // carEntry의 값(차량 객체)을 Car 타입으로 형변환하여 car에 저장합니다  //getValue()는 Car객체임
			car.print();
		}
		System.out.println("---------------------");
...
//		carMap.keySet();
		...
		while(carEntryIterator.hasNext()) {  	
			Entry carEntry=(Entry)carEntryIterator.next();  // entry객체는 키와 값을 배열로 받는다고 생각해라
			String key = (String)carEntry.getKey(); 		// key는 equals로 비교한다
			Car car=(Car)carEntry.getValue();
			...
			//한마디로 CarEntry의키와 값 요소를 순차적으로 가져온다는거!!!
		}
