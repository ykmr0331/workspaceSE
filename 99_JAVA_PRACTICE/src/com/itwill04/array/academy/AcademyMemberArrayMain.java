...
public class AcademyMemberArrayMain {
	...
	public static void main(String[] args) {
		...
		*/
		...
		System.out.println("2.번호 2   번 AcademyMember 여러명 출력");
		for (int i = 0; i < members.length; i++) {
		...
		for (int i = 0; i < members.length; i++) {
			if(members[i] instanceof AcademyStudent) {
				AcademyStudent stuMember=(AcademyStudent)members[i];
				if(stuMember.getBan().equals("자바")) {
					stuMember.print();
				}		
			}
		}
		...
		for (int i = 0; i < members.length; i++) {
			if(members[i] instanceof AcademyGangsa) {
				AcademyGangsa gangMember=(AcademyGangsa)members[i];
				if(gangMember.getSubject().equals("자바")) {
					gangMember.print();
				}		
			}
		}
		...
		for (int i = 0; i < members.length; i++) {
			if(members[i] instanceof AcademyStaff) {
				AcademyStaff staffMember=(AcademyStaff)members[i];
				if(staffMember.getDepart().equals("영업")) {
					staffMember.print();
				}		
			}
		}
